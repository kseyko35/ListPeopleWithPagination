package com.kseyko.listpeoplewithpagination.source
...
data class PersonModel(val people: List<Person>, val next: String?, val errorMessage: String?)

data class FetchResponse(val people: List<Person>, val next: String?)

data class FetchError(val errorDescription: String,val errorTypes: ErrorTypes?)
...
class  DataSource @Inject constructor () {
    ...
    private fun processRequest(next: String?): ProcessResult {
        ...
        if (isError) {
            ...
            error = FetchError(errorDescription = "Internal Server Error",ErrorTypes.INTERNAL_SERVER_ERROR)
        } else {
            ...
            if (next != null && (nextIntValue == null || nextIntValue < 0)) {
                error = FetchError(errorDescription = "Parameter error",ErrorTypes.PARAMETER_ERROR)
            } else {
                ...
                if (beginIndex > 0 && RandomUtils.roll(probability = Constants.backendBugTriggerProbability)) {
                    ...
                    responseNext = null
                    error = FetchError(errorDescription = "Failed to fetch data from source",ErrorTypes.NO_DATA)
                }
